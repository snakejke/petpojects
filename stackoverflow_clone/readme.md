...
# Авторизация и тест своих API с использованием JWT токена и POSTMAN 

#### Для получения JWT токена необходимо:
- Вручную добавить пользователя.

Или
- Зарегистрировать пользователя через RegistrationController.

После создания пользователя, можно получить токен для авторизации.

#### Получение токена:
1. Запустите проект и POSTMAN.
2. В POSTMAN необходимо отправить POST-запрос, в теле которого должны находиться ключи 
"login" и "pass" они содержат в себе логин и пароль пользователя.
Для передачи параметров в тело запроса перейдите во вкладку -> "Body", а затем выберите режим отправки данных -> "raw"(сырой). 
3. Если всё выполнено верно, в ответе вам придет json с токеном, который необходимо скопировать.
![](src/main/resources/static/images/jwt_tutor/jwt_tutor_1.png)

#### Получение доступа к защищенному API:
1. В POSTMAN необходимо указать метод запроса и url.
2. Далее перейдите во вкладку "Authorization" и выберите тип авторизации -> 
"Bearer Token", справа появится поле для ввода, туда нужно вставить раннее полученный и скопированный токен.
![](src/main/resources/static/images/jwt_tutor/jwt_tutor_2.png)

3. Если всё выполнено верно, пользователь получит доступ к запрашиваемому API. 
Запрос пройдёт корректно.
![](src/main/resources/static/images/jwt_tutor/jwt_tutor_3.png)
